...
- [3. 環境構築](#3-環境構築)
  ...
  - [3.3. （オプション）PDF ファイルを検索できるようにする](#33-オプションpdf-ファイルを検索できるようにする)
    - [3.3.1. 外部モジュールのインストール](#331-外部モジュールのインストール)
    - [3.3.2. Grep Office の検索設定を変更](#332-grep-office-の検索設定を変更)
- [4. 使い方](#4-使い方)
...
### 3.3. （オプション）PDF ファイルを検索できるようにする

所要時間: 10 分ほど

デフォルトでは PDF ファイルの検索に対応していませんが、  
外部モジュールを PC へインストールすることで検索が可能になります。

Windows 環境の場合は下記の手順でインストールと設定の変更を行ってください。  

#### 3.3.1. 外部モジュールのインストール
...
MacOS 等の場合は展開したディレクトリを Terminal で開き、手順 4 から同様の手順で行ってください。
...
#### 3.3.2. Grep Office の検索設定を変更

1. ディレクトリ上の `setup.py` を開く。
2. `■ PDF の検索設定` とコメントで記述されている箇所に存在する定数 `DETECT_PDF_FILE` を下記のように `True` へ書き換えて保存する。
...
- Word ファイルの場合
  ...
  - Y: 左から○文字目
- Excel ファイルの場合
  - X: 読み込めたテキストをセルごとに行として並べた状態での○行目 ※1
  - Y: セルごとの左から○文字目
- Power Point ファイルの場合
  - X: スライド○枚目
  - Y: スライドごとの○文字目
...
※1 現行版では Excel を開いた際のシートや行順ではなく、編集した順にテキストが表示されます。改善の余地あり……😓
