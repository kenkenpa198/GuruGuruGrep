...
### 5.3. bat ファイルのショートカット作成

bat ファイルはショートカットからの実行も可能です。  
ショートカットを作成して、デスクトップやスタートメニューフォルダなど好きな場所に配置すると簡単に実行できて便利です。

## 6. 注意点

### 6.1. 検索するディレクトリはなるべく深めに設定する
...
cmd.exe の仕様によるもので動作には特に影響ないので、`N` を入力して送信するか、× を押してウィンドウを閉じて OK です。
