...
2. 画面の指示に従って、検索対象のディレクトリパスと検索条件の文字列を入力・送信する。
   1. 入力の際はコピペでも OK です。ディレクトリパスの指定はエクスプローラーのアドレスバーからコピペしてくるのがオススメです。
3. 検索条件の一覧を確認したら `Enter` キーを押す。
4. 検索が開始され、検索結果が下記のイメージで表示される。
...
これらのファイルの場合は読み込んだ文字列を結合してから検索するという仕様上、ソースのテキストの行頭や行末が行中になってしまうためです。

そのためこれらの正規表現を用いる際はご留意ください。  
仕様的にも改善は難しいかなぁと思っています🤔
