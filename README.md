...
```text
※1 MacOS は簡単な動作確認のみ行っています。

※2 利用するモジュールに下記の要件が存在するため。
    - glob モジュール > glob 関数の recursive オプション指定要件が 3.5 以上のため。
    - pdfminer.six モジュールの推奨要件が 3.6 以上のため。
```
...
デフォルトでは PDF ファイルの検索に対応していませんが、外部モジュールをインストールすることで検索が可能になります。
...
11. ツールを bat ファイル等から実行し、下記の内容を確認する。
    1. エラーなどが出力されないこと。
    2. 検索条件の入力後に表示される検索情報に `PDF ファイル     : 検索しない` と表示されていること。
